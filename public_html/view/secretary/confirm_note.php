    ...
    if($id > 0) {
        ...
        if($note) {
            ...
            if($note_status == NoteStatus::VALIDATED) {
                $response['msg'] = "Désolé mais vous ne pouvez plus confirmer cette note. Elle a déjà été validée par un administrateur.";
            } else {
                $message = $noteModel->updateStatus($id, NoteStatus::CONFIRMED);
                if($message == Message::SUCCESS_MSG) {
                    $response['msg'] = "Note confirmée avec succès.";
                }
            }
        } else {
            $response['msg'] = "Désolé mais vous ne pouvez pas confirmer cette note. Si vous pensez qu'il s'agit d'une erreur, merci de contacter les responsables.";
        }
    } else {
