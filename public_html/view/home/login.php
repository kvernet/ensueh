...
->add('</div>')
->add('</div>')
->add('En cliquant sur <b>Se connecter</b>, vous acceptez nos <a href="terms">conditions</a>.')
...
->add('<div class="mb-4">')
->add('<div data-mdb-input-init class="form-outline">')
->add('<span>Pas de compte ? <a href="direction">Contactez les responsables</a> pour faire une demande d\'accès sur le portail de ' . APP_NAME . '!</span>')
->add('</div>')
->add('</div>')
->add('</div>')
->add('</form>')
...
->add('if (checkLength(user_name, 1, "Votre identifiant est obligatoire.", details)) {')
->add('if (checkLength(pwd, 1, "Votre mot de passe est obligatoire.", details)) {')
